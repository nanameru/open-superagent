...
// 3段階のReasoning(推論)を経て学術的レポートを生成する例
export async function generateDetailedArticle(
  ...
  try {
    ...
    if (!testResponse.ok) {
      throw new Error(`Gemini API connection test failed: ${testResponse.statusText}`);
    }
    console.log("Gemini API connection test successful");
  } catch (error) {
    console.error("Gemini API connection test failed:", error);
    throw new Error(`Gemini API connection failed: ${error}`);
  }
  ...
  while (attempt < MAX_RETRIES) {
    ...
    try {
      console.log("Setting up messages for Gemini generation...");
      ...
      const response = await fetch(
        `${GEMINI_API_ENDPOINT}?key=${GEMINI_API_KEY}`, 
        {
          ...
          body: JSON.stringify({
            contents: [{
              parts: [{
                text: `あなたは学術的な文章を書くAIアシスタントです。以下の要件と構成に従って、入力データと検索クエリに基づく学術的レポートを作成してください。
...
例えば、1番目の参考文献を引用する場合は必ず [1] を使用し、2番目の参考文献には必ず [2] を使用するようにしてください。`}]
      ...
      if (data.candidates && data.candidates[0].content) {
        ...
        onChunk({ content: generatedContent });
      } else {
        console.warn('No content generated by Gemini');
        onChunk({ content: '記事の生成に失敗しました。' });
      }
      ...
      if (attempt === MAX_RETRIES) {
        console.error('All attempts to generate article failed');
        throw error;
      }
    }
  }
